...
class JournalDataProvider
{
    ...
    collectNotFinishedTasks( curPath, dayName ){

        var _tmpArray = [];

        fs.readFileSync( curPath, 'utf8').toString().match(/^-\s\[\s\]\s.+$/gm).forEach( function (str, index) {
            _tmpArray.push(str);
        });

        this.todoArray.push({day: dayName, path: curPath, tasks: _tmpArray});

    }

    addNotFinishedTasks( rootFolder ){
        var _tmpArray = [
            '## Not finished tasks:',
            'Please, do not edit this file, because it regenerated automaticaly' ]
        this.todoArray.forEach(function (day){
            _tmpArray.push(`# [{$day.day}]({$day.path})`);
            _tmpArray.push('');
        ...
        fs.writeFileSync(rootFolder + '/todos.md', _tmpArray);
        this.add( rootFolder, rootFolder + '/todos.md');
    }
    ...
    add( rootFolder, entryPath )
    {
        ...
        if( isNote || isEntry )
        {
            ...
            if (
                isEntry
                && vscode.workspace.getConfiguration( 'vscode-journal-view' ).collectNotFinishedTasks
            )
            {
                this.collectNotFinishedTasks( fullPath, dayName );
            }
